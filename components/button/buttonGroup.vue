<template>
    <div class="r-button-group d-flex h-center" v-click-outside="handleClose">
        <div class="btn-container">
            <r-btn :class="{'r-button-group-active':item===modelValue}"
                   @click.prevent="emit(k)" :icon="icon" text v-for="(item,k) in items" :key="k">
                <slot :item="item">
                    <r-icon :exact="exact" v-if="icon" :class="isSelect?item['icon']:item"></r-icon>
...
export default {
  ...
  methods: {
    ...
    listInput (e) {
      ...
      this.$emit('update:modelValue', { menu: this.selected, item: e[0] })
      this.open = false
    }
  }
}
